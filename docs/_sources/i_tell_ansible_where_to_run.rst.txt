...
Suppose I have a bunch of raspberry pi computers on my network, I might want to name their group ``scotty`` in my `Ansible <http://docs.ansible.com/>`_ inventory.  They all have the same username, but they are on addresses, 192.168.0.71 and 192.168.0.169.  So, on the Linux machine that I will run my deployment scripts from, I would edit the ``/etc/ansible/hosts`` file like this:
